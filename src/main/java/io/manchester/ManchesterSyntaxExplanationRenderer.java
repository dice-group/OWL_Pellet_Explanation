...
public class ManchesterSyntaxExplanationRenderer implements ExplanationRenderer {
	...
	public void renderSingleExplanation(Set<OWLAxiom> explanation) throws OWLException,
			IOException {
		writer.printSpace();
		writer.printSpace();
		writer.printSpace();
	...
	//Newly added code for all explanations
	public void renderAllExplanations(OWLAxiom axiom, Set<Set<OWLAxiom>> explanations) throws OWLException, IOException {
		setCurrentAxiom(axiom);

		// Reset the index for rendering
		index = 1;
		...
		// Loop through all explanations and render each one
